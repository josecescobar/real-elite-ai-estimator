// Branded estimate PDF generation, shared by the owner-only download route and
// the public (token-gated) customer download on the share page.

import PDFDocument from "pdfkit";
import { estimateTotals, lineItemBreakdown, type TotalsInput } from "./estimate-calculations";
import type { CompanyProfile } from "./company";

export interface PdfLineItem extends TotalsInput {
  name: string;
  unit: string;
}
...
/** Render a branded estimate PDF and resolve to the completed Buffer. */
export function buildEstimatePdf(estimate: PdfEstimate, company: CompanyProfile): Promise<Buffer> {
...
/** Sanitize a job name into a safe, header-injection-free PDF filename slug. */
export function pdfFilename(jobName: string): string {
  const safeJob =
    jobName.replace(/[^a-zA-Z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "estimate";
  return `estimate-${safeJob}.pdf`;
}
